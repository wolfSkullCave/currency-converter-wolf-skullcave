...
def convert(num_to_convert, currency: str):
    if currency == "USD" or currency == "usd":
        print(f"${round_up(num_to_convert)} = R {round_up(num_to_convert * get_conversion_rate(url_dollar_array))}")
    elif currency == "EUR" or currency == "eur":
        print(f"€{round_up(num_to_convert)} = R {round_up(num_to_convert * get_conversion_rate(url_euro_array))}")
    else:
